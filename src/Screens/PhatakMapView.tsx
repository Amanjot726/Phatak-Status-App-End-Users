...
export default function PhatakMapViews({navigation}:any) {
    ...
    const setCurrentLocation = async () => {
      ...
      mapRef.current.animateToRegion(currentRegion, 3 * 500);
    }
    ...
    let showAllMarkers = () => {
      ...
      mapRef.current.fitToSuppliedMarkers(idList,{ edgePadding: 
        {top: 250,
          right: 150,
          bottom: 250,
          left: 150}
    ...
    let markersList = () => {
      return crossingList.map((element, index)=> 
        <Marker key={index}
            ...
            description={(element["trafficStatus"] == "Low" ? 'No Traffic' : element["trafficStatus"] == 'Medium' ? 'Moderate Traffic' : 'Heavy Traffic')
            +', Distance: ' + Math.round(((getDistanceFromLatLonInKm(region.latitude, region.longitude, parseFloat(element['location']['latitude']), parseFloat(element['location']['longitude']))+0.55)+ Number.EPSILON) * 10)/10 + ' km'}
            // pinColor= {element["phatakStatus"] == 0 || element["phatakStatus"] == 1 ? "green" : "red"}
            pinColor = {element["phatakStatus"] == 0 ? '#094f03' : element["phatakStatus"] == 1 ? '#DAA700' : element["phatakStatus"] == 2 ? '#E26D00' : '#DB1A00'}
            tappable
            identifier={'mk' + index}
        >
        </Marker>
